...
def intern_dashboard():
    try:
        ...
        leaderboard = sorted([u for u in all_users if u.get('role') != 'admin'], key=lambda x: x.get('total_raised', 0), reverse=True)[:5]
...
def admin_dashboard():
    ...
    leaderboard = sorted([u for u in all_users if u.get('role') != 'admin'], key=lambda x: x.get('total_raised', 0), reverse=True)
    
    return render_template('admin_dashboard.html', admins=admins, interns=interns, ambassadors=ambassadors, 
                           leaderboard=leaderboard, total_platform_raised=total_platform_raised)
